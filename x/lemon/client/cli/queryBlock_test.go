...
func networkWithBlockObjects(t *testing.T, n int) (*network.Network, []*types.Block) {
	...
	for i := 0; i < n; i++ {
		state.BlockList = append(state.BlockList, &types.Block{Creator: "ANY", Id: uint64(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Block)
			}
		})
	}
}
...
func TestListBlock(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.Block[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Block)
	})
}
